...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        main_print(f"timesteps {timesteps}, timesteps_prev {timesteps_prev}")
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            model_pred = transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask,guidance=guidance_tensor)[0] #43200
        ...
        with torch.no_grad():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                teacher_output = teacher_transformer(x,timesteps,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask,guidance=guidance_tensor)[0].float()
        ...
        with torch.no_grad():
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                # else:
                target_pred = teacher_transformer(x_prev_list,timesteps_prev,None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask,guidance=guidance_tensor)[0] #43200
...
def main(args):
    ...
    train_dataset = LatentDataset(args.data_json_path, args.num_latent_t, -1)
